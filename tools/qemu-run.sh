...
# Respect environment override; otherwise leave empty and we will try to detect below
OVMF_CODE="${OVMF_CODE:-}"
OVMF_VARS="${OVMF_VARS:-}"

uname_s=$(uname -s 2>/dev/null || echo Unknown)
if [[ -z "${OVMF_CODE}" && "$uname_s" == "Darwin" ]]; then
  echo "[qemu-run] macOS detected. If QEMU is installed via Homebrew, OVMF is usually at:" >&2
...
# Autodetect common firmware paths if not explicitly set or missing
...
# Try to locate a VARS template for writable NVRAM; we'll copy it into build/ and use as pflash.
...
if [[ ! -f "${OVMF_CODE:-}" ]]; then
  echo "[qemu-run] ERROR: OVMF_CODE not found." >&2
